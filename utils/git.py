...
def bytes_join(*args):
    ...
    if any(isinstance(I,bytes) for I in args):
        return b"".join(I if isinstance(I,bytes) else I.encode()
                        for I in args);
    else:
        return "".join(args);
...
def git_output(args,mode=None,null_stderr=False,input=None):
    ...
    if null_stderr:
        with open("/dev/null") as F:
            o = subprocess.check_output(['git',] + args,stderr=F,input=input);
    else:
...
def git_output_to_file(args,file):
    ...
    return subprocess.check_call(['git',] + args,stdout=file);
...
def git_commit_id(thing,fail_is_none=False):
    ...
    return git_ref_id(bytes_join(thing,"^{commit}"),fail_is_none=fail_is_none);

def git_root():
    """Return the top of the source directory we are currently in"""
    res = git_output(["rev-parse",
                      "--is-inside-work-tree",
                      "--is-inside-git-dir",
                      "--is-bare-repository",
                      "--absolute-git-dir",
...
def git_read_object(obj_type,idish):
    ...
    return GitObject(keys=dkeys,
                     raw_keys=keys,desc=desc);

def git_trailers(commit):
    """Return a list of trailers from a commit message"""
    res = [];
    for I in git_output(["show","--no-patch","--pretty=%(trailers:only,unfold)",commit],
                        mode="lines"):
        if not I.strip():
            continue;
        I = I.partition(b":");
...
def extract_date(s):
    ...
    tzoffset = datetime.timedelta(minutes=int(g.group(2) + b"1") *
...
def get_remote_branches():
    ...
    branches = git_output(["branch","--all","--list","--format",'%(refname)'],mode="lines");
...
class GitRange(object):
    ...
    def get_changed_files(self):
        ...
        diff = git_output(["diff-tree","-r",self.newest,self.ancestor],mode="lines");
    ...
    def get_commit_list(self,extra_args=[]):
        ids = git_output(["rev-list"] + extra_args + self.rev_range(),mode="lines");
        res = [];
        for I in ids:
    ...
    def sanity_check(self):
        ...
        if count >= 100:
            raise ValueError("Too many commits (%u) between %s ^%s probably a mistake"%(
...
def establish_ko_ssh(always_prompt=False):
    ...
    try:
        res = subprocess.check_output(["ssh", config.ko_ssh_server,
                                       "2fa","isval"]);
    ...
    subprocess.check_call(["ssh","git@gitolite.kernel.org","2fa","val-session",
                           keys]);
...
def compile_test(dot_config,mfiles=None):
    ...
    make = ["make","CC=" + config.compiler,"HOSTCC=" + config.compiler,"-s","-j8"];
    shutil.copyfile(dot_config,".config");
    subprocess.check_call(make + ["oldconfig"]);
    print("Running test compile using %r and %s"%(dot_config,config.compiler));
