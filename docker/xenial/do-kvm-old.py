    ...
    with open("/proc/meminfo") as F:
        for I in F:
            ...
            if tag == "MemTotal":
                ...
                if value[1] == "kB":
                    qemu_args["-m"] = "%uM" % (
                        (int(value[0]) * 1024 * 0.70) / 1024 / 1024)
                    return
...
def setup_fs(paths=()):
    ...
    os.symlink("/run/systemd/resolve/resolv.conf",
               os.path.join(upper, "resolv.conf"))

    subprocess.check_call([
        "mount", "-t", "overlay", "-o",
        "lowerdir=/etc,upperdir=%s,workdir=%s" % (upper, work), "none",
    ...
    for I in paths:
        for J in prefixes:
            if I.startswith(J):
                to_mount.add(J)
    for I in sorted(to_mount, key=lambda x: (len(x), x), reverse=True):
        create_mount(I)
    ...
    # Setup plan9 virtfs
    qemu_args[
        "-fsdev"] = "local,id=host_fs,security_model=passthrough,path=%s" % (
            mnt)
...
def set_kernel(tree):
    ...
    qemu_args.update({
        ...
        "-append":
        'root=/dev/root rw rootfstype=9p rootflags=trans=virtio console=ttyS0'
    })
...
def fixup_ipv6(mac, tap):
    ...
    mac = mac.split(':')
    addrs = set()
    addrs.add(":".join(("33", "33", "ff", mac[3], mac[4], mac[5])))
    # ff02::1:ff<mac>
    addrs.add("33:33:00:01:00:03")
    # ff02::1:3
    addrs.add("01:00:5e:00:00:fc")
    # 224.0.0.252
    for I in addrs:
        subprocess.check_call([
            "ip", "netns", "exec", "kvm", "ip", "maddress", "add", I, "dev",
            tap
        ])
...
def mac_to_host(mac):
    ...
    ipv6 = "fe80::%02x%02x:%02xff:fe%02x:%02x%02x" % (
        mac[0] ^ 0x2, mac[1], mac[2], mac[3], mac[4], mac[5])
...
def setup_net():
    ...
    master = get_ip_tag(
        subprocess.check_output(["ip", "route", "get", "8.8.8.8"]), "dev")
    master_mac = get_ip_tag(
        subprocess.check_output(["ip", "link", "show", "dev", master]),
        "link/ether")
    ...
    subprocess.check_call([
        "ip", "netns", "exec", "kvm", "ip", "link", "set", "dev", tap,
        "address", ":".join(tap_if_mac)
    ])

    tap_if = subprocess.check_output(
        ["ip", "netns", "exec", "kvm", "ip", "link", "show", "dev", tap])
    ...
    subprocess.check_call(
        ["ip", "netns", "exec", "kvm", "ip", "link", "set", "up", "dev", "lo"])
    subprocess.check_call(
        ["ip", "netns", "exec", "kvm", "ip", "link", "set", "up", "dev", tap])
    ...
    qemu_args["-device"].add(
        "virtio-net-pci,netdev=network0,mac=%s" % (tap_if_mac))
