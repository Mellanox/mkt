...
def setup_fs(paths=()):
    ...
    for dfn,v in get_mtab().items():
        ...
        qemu_args["-fsdev"].add("local,id=host_bind_fs%u,security_model=passthrough,path=%s"%(cnt,dfn));
        qemu_args["-device"].append("virtio-9p-pci,fsdev=host_bind_fs%u,mount_tag=bind%u"%(cnt,cnt));

        unit = b"%s.mount"%(subprocess.check_output(["systemd-escape","--path",dfn]).strip());
...
"""%(cnt,dfn));
        try:
            os.symlink(srv,b"/etc/systemd/system/local-fs.target.wants/%s"%(unit));
...
def set_kernel(tree):
    ...
    ver = get_ip_tag(subprocess.check_output(["file",bzimage]).decode(),"version");
    ...
    qemu_args.update({
        ...
        "-append": 'root=/dev/root rw ignore_loglevel rootfstype=9p rootflags=trans=virtio earlyprintk=serial,ttyS0,115200 console=hvc0'
    });

def set_loop_network(args):
    """Simple network that forwards our localhost port 4444 to the kvm's ssh port"""

    qemu_args["-netdev"].add(
              "bridge,br=br0,id=net0");
    qemu_args["-device"].append(
              "virtio-net,netdev=net0,mac="+args.mac);
...
def setup_from_pickle(args,pickle_params):
    ...
    write_once("/etc/shadow",
               "{user}:x:17486:0:99999:7:::\n".format(**p));
    write_once("/etc/group",
               "{user}:x:{gid}:\n".format(**p));
    write_once("/etc/sudoers",
               "{user} ALL=(ALL) NOPASSWD:ALL\n".format(**p));

    # Autologin as the given user
    with open("/etc/systemd/system/serial-getty@hvc0.service.d/override.conf","wt") as F:
        F.write("""[Service]
ExecStart=
