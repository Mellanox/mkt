...
def args_drvt_gui(parser):
    ...
    parser.add_argument(
        "GUI_ARGS", nargs="*", help="Arguments to pass to driver_test GUI, place them after a --")
...
def cmd_drvt_gui(args):
    ...
    if args.devel:
        ...
        dargs.extend([
            "-v",
            "%s:/usr/lib64/python2.7/site-packages/onetest:ro" %
            (os.path.join(args.devel,"onetest"))
        ])

    # Current driver test obnoxiously requires X even when running headless
