...
def args_make(parser):
    ...
    parser.add_argument("arch",
                        ...
                        choices=sorted([
                            "i386", "x86_64", "alpha", "arm",
                            "arm64","avr32", "blackfin", "c6x",
                            "cris", "frv", "h8300", "hexagon",
                            "ia64", "m32r", "m68k", "microblaze",
                            "mips", "mn10300", "openrisc",
                            "parisc", "powerpc", "s390",
                            "score", "sh", "sh64", "sparc",
                            "sparc32", "sparc64", "tile",
                            "tilepro", "tilegx", "um",
                            "unicore32", "xtensa"
                        ]));
    ...
    parser.add_argument("--dot-config",
                        dest="file",
                        help="Path to .config");
...
def cmd_make(args):
    ...
    if ((args.gcc is None and args.arch is not None) or
        (args.gcc is not None and args.arch is None)):
            exit("Please provide both GCC version and ARCH for cross-compile.")

    cmd = [ "sudo", "docker", "run", "-t", "--tmpfs", "/build:exec",
            "--rm", "-v", section['linux'] + ":/kernel:ro",
            "-v", section['ccache'] + ":/ccache"];
