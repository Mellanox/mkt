...
def get_simx_rdma_devices():
    return ['cx4-ib', 'cx4-en', 'cx5-ib', 'cx5-en',
            'cib', 'cx4lx', 'cx6-ib', 'cx6-en'];
...
def switch_to_vfio(bdf,modalias):
    ...
    with open("/sys/bus/pci/drivers/vfio-pci/remove_id","wb",buffering=0) as F:
...
def get_mac():
    ...
    if not mac:
        ...
        mac = "52:54:" + format(a, '02x') + ":" + format(b, '02x') + ":" + format(c, '02x') + ":" + format(d, '02x');
...
def cmd_run(args):
    ...
    union = set(get_simx_rdma_devices()).union(set(get_pci_rdma_devices().keys()));
    if not set(s).issubset(union):
        ...
        exit("There is an error in configuration file, PCI and/or SIMX devices don't exists.");
    ...
    if args.pci:
        subprocess.check_call([
            "sudo",
            sys.argv[0],
            "vfio-enable"] + ["--pci=%s"%(I) for I in args.pci]);
    ...
    if not os.path.isdir(args.kernel):
        raise ValueError("Kernel path is not a directory/does not exist"%(args.kernel));
    ...
    src_dir = os.path.dirname(os.path.abspath(inspect.getfile(
        inspect.currentframe())));

    docker_exec(["run"] + mapdirs.as_docker_bind() + [
                 "-v","%s:/plugins:ro"%(src_dir),
                 "--rm",
                 "--net=host",
	         "--privileged",
	         "--name=%s"%(args.os),
                 "--tty",
                 "-e","KVM_PICKLE=%s"%(get_pickle(args)),
                 "--interactive",
                 "lab/kvm:%s"%(args.os)] + do_kvm_args);
